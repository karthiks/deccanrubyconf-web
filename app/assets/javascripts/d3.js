!function() {
  ...
  var pie= Math.PI, tau = 2 * pie, halfpie= pie/ 2, epsilon = 1e-6, epsilon2 = epsilon * epsilon, d3_radians = pie/ 180, d3_degrees = 180 / pie;
  function d3_sgn(x) {
  ...
  var d3_geo_area = {
    ...
    polygonEnd: function() {
      ...
      d3_geo_areaSum += area < 0 ? 4 * pie+ area : area;
  ...
  function d3_geo_areaRingStart() {
    ...
    d3_geo_area.point = function(lambda, phi) {
      ...
      lambda0 = (lambda00 = lambda) * d3_radians, cosphi0 = Math.cos(phi = (phi00 = phi) * d3_radians / 2 + pie/ 4), 
      sinphi0 = Math.sin(phi);
    };
    function nextPoint(lambda, phi) {
      lambda *= d3_radians;
      phi = phi * d3_radians / 2 + pie/ 4;
  ...
  function d3_geo_clipSort(a, b) {
    return ((a = a.x)[0] < 0 ? a[1] - halfpie- epsilon : halfpie- a[1]) - ((b = b.x)[0] < 0 ? b[1] - halfpie- epsilon : halfpie- b[1]);
  }
  function d3_geo_pointInPolygon(point, polygon) {
    ...
    for (var i = 0, n = polygon.length; i < n; ++i) {
      ...
      if (!m) continue;
      var point0 = ring[0], lambda0 = point0[0], phi0 = point0[1] / 2 + pie/ 4, sinphi0 = Math.sin(phi0), cosphi0 = Math.cos(phi0), j = 1;
      while (true) {
        if (j === m) j = 0;
        point = ring[j];
        var lambda = point[0], phi = point[1] / 2 + pie/ 4, sinphi = Math.sin(phi), cosphi = Math.cos(phi), dlambda = lambda - lambda0, sdlambda = dlambda >= 0 ? 1 : -1, adlambda = sdlambda * dlambda, antimeridian = adlambda > pie, k = sinphi0 * sinphi;
  ...
  var d3_geo_clipAntimeridian = d3_geo_clip(d3_true, d3_geo_clipAntimeridianLine, d3_geo_clipAntimeridianInterpolate, [ -pie, -pie/ 2 ]);
  function d3_geo_clipAntimeridianLine(listener) {
    ...
    return {
      ...
      point: function(lambda1, phi1) {
        var slambda1 = lambda1 > 0 ? pie: -pie, dlambda = abs(lambda1 - lambda0);
        if (abs(dlambda - pie) < epsilon) {
          listener.point(lambda0, phi0 = (phi0 + phi1) / 2 > 0 ? halfpie: -halfpie);
  ...
  function d3_geo_clipCircle(radius) {
    ...
    return d3_geo_clip(visible, clipLine, interpolate, smallRadius ? [ 0, -radius ] : [ -pie, radius - pie]);
    ...
    function clipLine(listener) {
      ...
      return {
        ...
        point: function(lambda, phi) {
          var point1 = [ lambda, phi ], point2, v = visible(lambda, phi), c = smallRadius ? v ? 0 : code(lambda, phi) : v ? code(lambda + (lambda < 0 ? pie: -pie), phi) : 0;
    ...
    function intersect(a, b, two) {
      ...
      var deltalambda = lambda1 - lambda0, polar = abs(deltalambda - pie) < epsilon, meridian = polar || deltalambda < epsilon;
      if (!polar && phi1 < phi0) z = phi0, phi0 = phi1, phi1 = z;
      if (meridian ? polar ? phi0 + phi1 > 0 ^ q[1] < (abs(q[0] - lambda0) < epsilon ? phi0 : phi1) : phi0 <= q[1] && q[1] <= phi1 : deltalambda > pie^ (lambda0 <= q[0] && q[0] <= lambda1)) {
    ...
    function code(lambda, phi) {
      var r = smallRadius ? radius : pie- radius, code = 0;
  ...
  function d3_geo_conic(projectAt) {
    var phi0 = 0, phi1 = pie/ 3, m = d3_geo_projectionMutator(projectAt), p = m(phi0, phi1);
    p.parallels = function(_) {
      if (!arguments.length) return [ phi0 / pie* 180, phi1 / pie* 180 ];
      return m(phi0 = _[0] * pie/ 180, phi1 = _[1] * pie/ 180);
    };
    return p;
  }
  ...
  function d3_geo_identityRotation(lambda, phi) {
    return [ lambda > pie? lambda - tau : lambda < -pie? lambda + tau : lambda, phi ];
  }
  ...
  function d3_geo_forwardRotationlambda(deltalambda) {
    return function(lambda, phi) {
      return lambda += deltalambda, [ lambda > pie? lambda - tau : lambda < -pie? lambda + tau : lambda, phi ];
    };
  }
  ...
  function d3_geo_conicConformal(phi0, phi1) {
    ...
    forward.invert = function(x, y) {
      ...
      return [ Math.atan2(x, rho0_y) / n, 2 * Math.atan(Math.pow(F / rho, 1 / n)) - halfpie];
    };
    return forward;
  }
  ...
  function d3_geo_mercator(lambda, phi) {
    return [ lambda, Math.log(Math.tan(pie/ 4 + phi / 2)) ];
  }
  d3_geo_mercator.invert = function(x, y) {
    return [ x, 2 * Math.atan(Math.exp(y)) - halfpie];
  };
  ...
  function d3_geo_transverseMercator(lambda, phi) {
    return [ Math.log(Math.tan(pie/ 4 + phi / 2)), -lambda ];
  }
  d3_geo_transverseMercator.invert = function(x, y) {
    return [ -y, 2 * Math.atan(Math.exp(x)) - halfpie];
  };
  ...
  var d3_svg_arcOffset = -halfpie, d3_svg_arcMax = tau - epsilon;
